...
class Register(Watch_History):
	...
	def Register_In_JSON(self):
		...
		for current_dict in titles:
			...
			# Define a local list of languages as a copy of the original list of small languages
			languages = deepcopy(self.languages["Small"])

			# Add the "ja" (Japanese) language
			languages.append("ja")

			# Remove language keys that match the original or romanized titles
			for language in languages:
				# If the language is inside the current dictionary
				if language in current_dict:
					# If the original title is the same as the current language title
					# Or the romanized title is present and it is the same as the current language title
					if (
						current_dict["Original"] == current_dict[language] or
						("Romanized" in current_dict and
						current_dict["Romanized"] == current_dict[language])
					):
						current_dict.pop(language)
		...
		# Get the "Comments" dictionary from the root "Comments.json" file
		...
		# If the comment "Write" state is True
		if self.dictionary["Comment Writer"]["States"]["Write"] == True:
			# Increment the number of comments of the media (item) "Watched" dictionary by one
	...
	def Define_Titles(self, language_parameter, language, titles_dictionary):
		...
		# If the title is not inside the list of titles
		if titles_dictionary[key] not in media_titles:
		...
		if (
			...
			# If the title is not inside the list of titles
			if titles_dictionary["Language"] not in media_titles:
			...
			for local_language in self.languages["Small"]:
				...
				# And if the title is not inside the list of titles
				if (
					local_language in titles_dictionary and
					titles_dictionary[local_language] != titles_dictionary["Language"] and
					titles_dictionary[local_language] not in media_titles
				):
		...
		if (
			language_parameter != "General" and
			language in titles_dictionary and
			titles_dictionary[language] not in media_titles
		):
			# Add the language title to the list of titles
			media_titles.append(titles_dictionary[language])

		# Iterate through the list of titles
		for title_number, title in enumerate(media_titles):
			...
			if (
				...
				# Remove them and update the root title
				media_titles[title_number] = title[2:]
	...
	def Check_Media_Status(self):
		...
		if self.media["States"]["Series media"] == True:
			# Create a shortcut to the list of media item episode titles in the media language
			media_item_episodes = self.media["Item"]["Episodes"]["Titles"][self.media["Language"]]

			# If the media has a list of media items
			# And the episode title is the last one
			if (
				self.media["States"]["Has a list of media items"] == True and
				self.media["Episode"]["Title"] == media_item_episodes[-1]
			):
				# And the media is not a YouTube channel
				if self.media["States"]["Video"] == False:
					# If the media item is the last one
					if self.media["Item"]["Title"] == self.media["Items"]["List"][-1]:
						# Define the media as completed
						self.media["States"]["Completed media"] = True

					# If the media item is not the last one (the media is not completed)
					if self.media["Item"]["Title"] != self.media["Items"]["List"][-1]:
						# Get the next media item title
						next_media_item_title = self.media["Items"]["List"][self.media["Item"]["Number"] + 1]

						# Create a local copy of the root dictionary
						dictionary_copy = deepcopy(self.dictionary)

						# Update the keys of the next media item dictionary
						self.media["Item"]["Next"] = self.Define_Media_Item(dictionary_copy, media_item = next_media_item_title)["Media"]["Item"]

						# Update the current media item file to be the next one
						self.File.Edit(self.media["Items"]["Folders"]["current"], self.media["Item"]["Next"]["Title"], "w")

					# Add the "Status" key and "Completed" value to the end of the media item "Details" dictionary
					key_value = {
			...
			if (
				...
				self.media["Episode"]["Title"] == media_item_episodes[-1] and
				self.media["States"]["Video"] == False
			):
				# Define the media as completed
				self.media["States"]["Completed media"] = True

			# If the media is not completed
			# And the current media item is not completed
			# And the total number of media item episodes is not one
			if (
				self.media["States"]["Completed media"] == False and
				self.media["States"]["Completed media item"] == False and
				len(media_item_episodes) != 1
			):
				# Try to get and define the next episode
				try:
					# Define the next episode
					self.media["Episode"]["Next"] = media_item_episodes[self.media["Episode"]["Number"]]

					# Define the current episode to be watched as the next one
					...
					text_to_write = self.Text.From_Dictionary(self.media["Item"]["Details"])

					# Update the media item "Details.txt" file with the updated media item "Details" dictionary
					self.File.Edit(self.media["Item"]["Folders"]["details"], text_to_write, "w")
		...
		if self.media["States"]["Completed media"] == True:
			...
			if (
				self.Language.language_texts["remote_origin"] in self.media["Details"] and
				self.media["Details"][self.Language.language_texts["remote_origin"]] == "YouTube"
			):
		...
		# If the media is a series media (not a movie)
		# And the media item "Type" dictionary exists
		# And the media item is a movie
		if (
			self.media["States"]["Series media"] == True and
			"Type" in self.media["Item"] and
			self.media["Item"]["Type"][self.language["Small"]] == self.language_texts["movie"]
		):
			# Get the list of movies that are on the "Plan to watch" watching status
			movies_list = self.Get_Media_List(self.media_types["Movies"], self.texts["plan_to_watch, title()"]["en"])

			# Iterate through the list of movie titles
			for movie_title in movies_list:
				# If the media item title is inside the movie title
				if self.media["Item"]["Title"] in movie_title:
					# Define the movie prototype dictionary
					movie_dictionary = {
						"Media type": self.media_types["Movies"], # The "Movies" media type dictionary
						"Media": {
							"Title": movie_title # The movie title
						}
					}

					# Define the media dictionary for the defined movie
					movie_dictionary = self.Select_Media(movie_dictionary)

					# Create a shortcut to the comments folder of the media item
					comments_folder = self.media["Item"]["Folders"]["comments"]["root"]

					# Create a shortcut to the comments folder of the movie
					movie_comments_folder = movie_dictionary["Media"]["Item"]["Folders"]["comments"]["root"]

					# Copy the contents of the media comments folder to the movie comments folder
					self.Folder.Copy(comments_folder, movie_comments_folder)

					# Change the status of the movie to "Completed"
					self.Change_Status(movie_dictionary)

		# If the media is not a series media (it is a movie)
		if self.media["States"]["Series media"] == False:
			# Create a shortcut to the list of plural English media types
			media_types = self.media_types["Plural"]["en"]

			# Define the list of media types to remove
			remove_list = [
				"Movies",
				"Videos"
			]
			...
			# Remove them
			for media_type in remove_list:
				media_types.remove(media_type)

			# Define the local list of series media
			series_media_list = {}

			# Define the list of statuses to use to get the media that has one of them
			statuses_list = [
				"On hold",
				"Watching",
				"Re-watching"
			...
			# Iterate through the local list of media types
			for plural_media_type in media_types:
				# Get the media type dictionary for the current media type
				media_type_dictionary = self.media_types[plural_media_type]

				# Get the list of media for the current media type that has one of the statuses in the list of statuses
				media_list = self.Get_Media_List(media_type_dictionary, statuses_list)

				# Iterate through the list of media
				for media_title in media_list:
					# Define the root folder as the root "Media information" folder of the current media type
					root_folder = media_type_dictionary["Folders"]["Media information"]["root"]

					# Define the media folder as the root folder plus the media title
					media_folder = root_folder + self.Sanitize_Title(media_title) + "/"

					# Define the media items folder as the media folder plus the plural subfolder of the media type
					media_items_folder = media_folder + media_type_dictionary["Subfolders"]["Plural"] + "/"

					# If the media items folder exists
					if self.Folder.Exists(media_items_folder) == True:
						# Define the media items file as the media items folder plus the plural subfolder of the media type
						media_items_file = media_items_folder + media_type_dictionary["Subfolders"]["Plural"] + ".txt"
						...
						for media_item_title in media_items:
							# Define the media item folder as the media items folder plus the media item title
							media_item_folder = media_items_folder + self.Sanitize_Title(media_item_title) + "/"

							# Define the media item "Details" file
							media_item_details_file = media_item_folder + self.Language.language_texts["details, title()"] + ".txt"

							# Get the media item details dictionary
							media_item_details = self.File.Dictionary(media_item_details_file)

							# Get the year of the root media item
							root_media_item_year = self.media["Item"]["Details"][self.Date.language_texts["year, title()"]]

							# Get the year of the local media item
							local_media_item_year = media_item_details[self.Date.language_texts["year, title()"]]

							# If the local media item title is equal to the root media item title (the one that was watched)
							# Or the root media item title split by a space and an opening parenthesis is inside the local media item title
							# And the year of the root media item (movie) is the same as the year of the local media item (non-movie)
							if (
								media_item_title == self.media["Item"]["Title"] or
								self.media["Item"]["Title"].split(" (")[0] in media_item_title and
								root_media_item_year == local_media_item_year
							):
								# Define the media prototype dictionary
								media_dictionary = {
									# The current media type dictionary (not "Movies" neither "Videos")
									"Media type": media_type_dictionary,
									"Media": {
										# The local media title
										"Title": media_title
									}
								}

								# Define the media dictionary for the local media
								media_dictionary = self.Select_Media(media_dictionary)

								# Define the media item as the current local media item
								...
								# Transform the media item "Details" dictionary into a text string
								text_to_write = self.Text.From_Dictionary(media_dictionary["Media"]["Item"]["Details"])

								# Update the media item "Details.txt" file with the updated media item "Details" dictionary
								self.File.Edit(media_dictionary["Media"]["Item"]["Folders"]["details"], text_to_write, "w")
	...
	def Check_Media_Dates(self):
		...
		if self.media["States"]["Completed media item"] == True:
			...
			# Transform the "Started watching" time into a date dictionary
			self.media["Item"]["Started watching"] = self.Date.From_String(self.media["Item"]["dates"][key], format = "%H:%M %d/%m/%Y")

			# Calculate the difference between the started and the finished watching times
			...
			# Define the time spent watching using the started and finished watching times
			self.media["Item"]["Time spent watching"] = difference["Text"][self.language["Small"]]

			# If the first two strings are a comma and a space, remove them
			...
			# Create a shortcut to the media "the item" text in the user language
			the_item_text = self.media["Texts"]["the_item"][self.language["Small"]]

			# If the media item is not a single unit one
			# And the lowercase media item type is inside the list of "Plural secondary types"
			if (
				self.media["States"]["Single unit"] == False and
				self.media["Item"]["Type"]["en"].lower() in self.secondary_types["Plural secondary types"]
			):
				# Change the "the item" text to its plural version
				the_item_text = self.media["Texts"]["Item (plural)"]["The"][self.language["Small"]]

			# Define the list of items to use to format the template
			items = [
				# The "How long did it take me to finish" text plus the " the item" text
				# Example:
				# "How long did it take me to finish" + " the season"
				# "How long did it take me to finish" + " the specials" (in plural)
				self.language_texts["how_long_did_it_take_me_to_finish"] + " " + the_item_text,
			...
			# Create a shortcut to the media item "Dates.txt" file
			...
			# Add the "the item" text to the "When I started watching" text
			...
			if (
				...
				# Add two line breaks and the "Finished watching text" to the Diary Slim dates "Media item" key
				self.dictionary["Entry"]["Diary Slim"]["Dates"]["Media item"] = "\n\n" + self.media["Item"]["Finished watching text"]
		...
		if (
			...
			self.media["Started watching"] = self.Date.From_String(self.media["dates"][key], format = "%H:%M %d/%m/%Y")
			...
			# Create a shortcut to the "the container" text in the user language
			the_container_text = self.media["Texts"]["Container texts"]["The"]

			# Define the list of items to use to format the template
			items = [
				# The "How long did it take me to finish" text plus the " the container" text
				# Example:
				# "How long did it take me to finish" + " the anime"
				self.language_texts["how_long_did_it_take_me_to_finish"] + " " + the_container_text,
			...
			# Create a shortcut to the media "Dates.txt" file
			...
			if len(lines) > 2:
				...
				while len(lines) > 2:
					# Remove the lines beyond the second line to remove the "finished watching" and "duration" lines
					# This is to keep only the "started watching" time and to add the new "finished watching" and "duration" lines
					lines.pop(2)

				# Update the "String" key to be a text version of the list of lines
				contents["String"] = self.Text.From_List(lines)
			...
			self.media["Finished watching text"] = self.media["Finished watching text"].replace(self.language_texts["when_i_started_watching"] + ":", self.language_texts["when_i_started_watching"] + " " + the_container_text + ":")
	...
	def Define_Diary_Slim_Text(self):
		...
		if self.media["States"]["Re-watching"] == True:
			# Replaced the "watching" text with the "re-watching [number of times]" text
			# 
			# Examples:
		...
		if self.media["States"]["Series media"] == True:
			...
			# Define the unit text as the media cover text in the user language
			...
			if (
				...
				# Create a shortcut to the list of titles
			...
			# Create a shortcut to the "of the" text template
			of_the_text_template = self.Language.language_texts["of_the_{}"]
			...
			if (
				...
				# Create a shortcut to the lowercase season text
				season_text = self.language_texts["season, title()"].lower()

				# Create a local texts dictionary
				texts = {
					"First": {},
					"Last": {}
				}

				# Iterate through the keys and dictionaries inside the local texts dictionary
				for text, dictionary in texts.items():
					# Iterate through the list of genders
					for gender in ["masculine", "feminine"]:
						# Define the text key with the text and gender
						# 
						# Examples:
						# "first, masculine"
						# "first, feminine"
						text_key = text.lower() + ", " + gender

						# Get the text using the gendered text key and add a space at the end
						gendered_text = self.Language.language_texts[text_key] + " "

						# Add the gendered text to the dictionary
						dictionary[gender.title()] = gendered_text

				# If the media item is not the root media
				if self.media["States"]["The media item is the root media"] == False:
					# If the media is not a video channel
					if self.media["States"]["Video"] == False:
						# Define the default media item addon and position key as empty strings
						media_item_addon = ""
						position_key = ""

						# If the media item is the first one
						if self.media["Item"]["Title"] == self.media["Items"]["List"][0]:
							# Change the position key to be "First"
							position_key = "First"

						# If the media item is the last one
						if self.media["Item"]["Title"] == self.media["Items"]["List"][-1]:
							# Change the position key to be "Last"
							position_key = "Last"

						# If the position key is not empty
						if position_key != "":
							# Define the media item addon as the feminine text in the position key
							media_item_addon = texts[position_key]["Feminine"]

						# Format the "of the" text template to add the media item addon and the "season" text
						of_the_text = of_the_text_template.format(media_item_addon + season_text)

						# Create a shortcut to the unit text
						unit_text = self.media["Texts"]["unit"][self.language["Small"]]
						...
						# Create a shortcut to the uppercase " Season" text
				...
				if (
					self.media["States"]["The media item is the root media"] == True and
					self.media["Item"]["Title"] == self.media["Items"]["List"][0]
				):
					# Create the media item "Texts" dictionary if it does not exist
					if "Texts" not in self.media["Item"]:
						self.media["Item"]["Texts"] = {
							# Create the "Item" dictionary
							"Item": {}
						}

					# Define the text gender to use as "feminine"
					gender = "Feminine"

					# ----- #

					# Define the text initially as "the " in the defined gender
					text = self.Language.language_texts["the, " + gender.lower()] + " "

					# Add the "first" (gendered) and "season" texts
					text += texts["First"][gender] + season_text

					# Add the "the first" text to the "Item" texts dictionary
					self.media["Item"]["Texts"]["Item"]["The first"] = text

					# ----- #

					# Define the text as "first season"
					text = texts["First"][gender] + season_text

					# Format the "of the" text template with the text defined above
					text = of_the_text_template.format(text)

					# Add the "of the first" text to the "Item" texts dictionary
					self.media["Item"]["Texts"]["Item"]["Of the first"] = text

					# ----- #

					# Add the "of the first season" text to the right of the unit ("episode") text
					# 
					# Example:
					# "this episode of the first season"
					watched_item_text = watched_item_text.replace(unit_text, unit_text + " " + text)
		...
		if self.media["States"]["Series media"] == False:
			# Create a shortcut to the "this" text
		...
		# If the media is not a series media (it is a movie)
		if self.media["States"]["Series media"] == False:
			# Add additional details about the movie that are present inside the parentheses of the original movie title
		...
		# If the user is re-watching the media (item)
		if self.media["States"]["Re-watching"] == True:
			# Add the number text of re-watched times in the user language
		...
		if (
			...
			"Link" in self.media["Episode"]["Remote"]
		):
		...
		# Define the first hashtags to add to the Diary Slim text
		...
		if (
			"Remote" in self.media["Episode"] and
			self.media["Episode"]["Remote"]["Title"] == "YouTube"
		):
			# Add the "YouTube" hashtag
			hashtags += " #YouTube"

		# Create a shortcut to the plural media type in the user language
		...
		# If the "Media" dates key inside the "Dates" dictionary is not an empty string
		if self.dictionary["Entry"]["Diary Slim"]["Dates"]["Media"] != "":
			self.dictionary["Entry"]["Diary Slim"]["Text"] += self.dictionary["Entry"]["Diary Slim"]["Dates"]["Media"]

			# Add the hashtags again
			self.dictionary["Entry"]["Diary Slim"]["Text"] += "\n\n" + hashtags

		# ---------- #

		# Import the "Diary_Slim" module
		module = importlib.import_module("." + "Diary_Slim", "Diary_Slim")

		# Get the class object
		self.Diary_Slim = getattr(module, "Diary_Slim")

		# Run the "Diary_Slim" class to define its variables
		self.Diary_Slim = self.Diary_Slim()

		# Create a shortcut to the finished watching date dictionary
		...
		# Add the finished watching time to the Diary Slim "Time" key
		self.dictionary["Entry"]["Diary Slim"]["Time"] = self.dictionary["Entry"]["Times"]["Finished watching"]["Formats"]["HH:MM DD/MM/YYYY"]
	...
	def Post_On_The_Social_Networks(self):
		...
		# Define the local media cover text to be used as "The episode cover"
		media_cover_text = self.language_texts["the_episode_cover"]

		# If the media is not a series media (it is a movie)
		if self.media["States"]["Series media"] == False:
			# Replace the "episode" text in the media cover text with the "movie" text
			media_cover_text = media_cover_text.replace(self.language_texts["episode"], self.language_texts["movie"])

		# If the media is a video channel
		if self.media["States"]["Video"] == True:
			# Replace the "episode" text in the media cover text with the "video" text
			media_cover_text = media_cover_text.replace(self.language_texts["episode"], self.Language.language_texts["video, title()"].lower())

		# Add the local media cover text to the root dictionary
		social_networks["Media cover text"] = media_cover_text

		# ---------- #

		# Define the "Text template" key as the "I posted the watched text" text template
		social_networks["Text template"] = self.language_texts["i_posted_the_watched_text, type: template"] + "."

		# Format the text template with the media cover text to create the posted text
		social_networks["Posted text"] = social_networks["Text template"].format(social_networks["Media cover text"])
		...
		if self.dictionary["Entry"]["States"]["Post on the social networks"] == True:
			...
			social_networks = {
				...
				"Custom links": {
					...
					"Discord": "https://discord.com/channels/311004778777935872/641352970352459776"
				},
				"Input text": ""
			}

			# Define the input text as to be about when the user finishes posting the status and the cover of the watched media on the current social network
			input_text = self.language_texts["press_enter_when_you_finish_posting_the_status_and_the_[media_cover], type: long"]

			# Replace the "[media_cover]" format string with the media cover text
			input_text = input_text.replace("[media_cover]", media_cover_text)

			# Define the input text inside the local social networks dictionary
			social_networks["Input text"] = input_text
	...
	def Write_On_Diary_Slim(self):
		...
		dictionary = {
			...
			"Time": self.dictionary["Entry"]["Diary Slim"]["Time"],
	...
	def Get_The_Media_Title(self, is_media_item = False, media_item = None, language = False, no_media_title = False):
		...
		if (
			...
			# If the media item title is not inside the media title
			if media_item_title not in media_title:
	...
	def Update_Statistic(self):
		...
		if "Items" in self.media:
			# Add the root media "Items" dictionary to the local media dictionary
	...
	def Show_Information(self):
		# Define the default header text as an empty string
		self.dictionary["Header text"] = ""

		# If the "Completed media" state is True
		# (The user finished watching the media)
		if self.media["States"]["Completed media"] == True:
			# Define the text as the selected "this container" text
