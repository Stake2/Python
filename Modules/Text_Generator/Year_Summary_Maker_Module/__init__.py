...
def Select_Language(language_to_select = None):
	...
	if selected_language == language_pt:
...
def Define_Year_Global_Texts():
	...
	media_comments_header = selected_year_summary_texts_array[i]
...
def Make_Media_Comments_Text():
	global media_comments_text
	global year_comment_number
	global last_comment_number
	global media_comments_header

	import Watch_History.Comment_Writer as Comment_Writer

	year_comment_number = Comment_Writer.Get_Comment_Number(comment_number_type = "year")[0]
	last_comment_number = Comment_Writer.Get_Comment_Number(comment_number_type = "year")[1].replace("\n", "")

	if year_comment_number == 1:
		media_comments_header = media_comments_header.replace("Comments", "Comment")
		media_comments_header = media_comments_header.replace("Comentários", "Comentário")

	media_comments_text = media_comments_header.format(year_comment_number, last_comment_number)
...
def Make_Year_Summary_Text():
	...
	full_year_summary_string += media_comments_text + "\n" + "\n"
