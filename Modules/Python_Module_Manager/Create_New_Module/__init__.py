...
class Create_New_Module(Python_Module_Manager):
	...
	def Ask_For_Module_Info(self):
		...
		self.english_language = full_languages_translated_dict[full_language_en][Language_Item_Definer(1, 2)]
		self.portuguese_language = full_languages_translated_dict[full_language_pt][Language_Item_Definer(1, 2)]
		self.english_and_portuguese_language = self.english_language + " " + Language_Item_Definer("and", "e") + " " + self.portuguese_language

		self.descriptions_text = Language_Item_Definer("descriptions", "descrições")

		self.in_two_lines_text = ", " + Language_Item_Definer("in two lines, in thar order", "em duas linhas, nesta ordem")

		self.show_text = self.type_the_text.format(self.descriptions_text, self.english_and_portuguese_language + self.in_two_lines_text) + ":"

		self.module_descriptions_prototype = Text_Writer(self.show_text, finish_text = "default_list", return_list = True, capitalize_lines = True, separator = ": ", enumerate_lines = True, stop_on_line = 2, second_space = False)
		...
		self.separated_by_lines_text = ", " + Language_Item_Definer("separated by lines", "separadas por linhas")

		self.show_text = self.type_the_text.format("classes", self.english_language + self.separated_by_lines_text) + ":"

		self.classes = Text_Writer(self.show_text, finish_text = "default_list", return_list = True, capitalize_lines = True, enumerate_lines = True, separator = ": ", second_space = False)
		...
		self.description_of_classes_text = Language_Item_Definer("descriptions of classes", "descrições das classes")

		for full_language in full_languages_not_none:
			full_language_translated = full_languages_translated_dict[full_language][Language_Item_Definer(1, 2)]

			self.show_text = self.type_the_text.format(self.description_of_classes_text, full_language_translated + self.separated_by_lines_text) + ":"

			self.class_descriptions[full_language] = Text_Writer(self.show_text, finish_text = "default_list", return_list = True, capitalize_lines = True, enumerate_lines = True, separator = ": ", stop_on_line = len(self.classes), second_space = False)

		i = 0
		for class_description in self.class_descriptions[full_language_en]:
			if class_description == "":
				self.class_descriptions[full_language_en][i] = Capitalize_First_Letter(self.classes[i].replace("_", " ").lower())
	...
	def Define_Variables(self):
		...
		Create_Folder(self.script_folder, self.global_switches)

		self.root_python_file = self.script_folder + "__init__.py"
		Create_Text_File(self.root_python_file, self.global_switches)

		self.script_main_class_folder = self.script_folder + self.module_name + "/"
		Create_Folder(self.script_main_class_folder, self.global_switches)

		self.main_class_python_file = self.script_main_class_folder + "__init__.py"
		Create_Text_File(self.main_class_python_file, self.global_switches)
		...
		for class_ in self.classes:
			...
			Create_Folder(class_folder, self.global_switches)

			init_file = class_folder + "__init__.py"
			Create_Text_File(init_file, self.global_switches)
			...
			self.class_descriptions_string += 'Language_Item_Definer("{}", "{}"),'.format(self.class_descriptions[full_language_en][i], self.class_descriptions[full_language_pt][i])
	...
	def Write_To_Files(self):
		self.python_root_code = self.python_root_code_template.format(self.module_name, self.import_classes_string, self.classes_string, self.class_descriptions_string, self.module_descriptions[full_language_en], self.module_descriptions[full_language_pt])
	...
	def Create_Module_Bat(self):
		...
		Create_Text_File(self.bat_file, self.global_switches)

		text_to_write = self.conemu_bat_template

		text_to_write = text_to_write.replace("[Name]", self.module_name.replace("_", " "))
		text_to_write = text_to_write.replace("[Module]", self.module_name.replace(" ", "_"))
	...
	def Show_Module_Info(self):
		...
		for full_language in full_languages_not_none:
			...
			print("\t" + full_language_translated + '"' + self.module_descriptions[full_language] + '",')
		...
		i = 0
		for class_ in self.classes:
			self.english_class_description = '"' + full_language_en + '": "' + self.class_descriptions[full_language_en][i] + '",'
			self.portuguese_class_description = '"' + full_language_pt + '": "' + self.class_descriptions[full_language_pt][i] + '",'

			if class_ != self.classes[0]:
				print()

			print("\t" + class_ + " = {")
			print("\t\t" + self.english_class_description)
			print("\t\t" + self.portuguese_class_description)
			print("\t" + "}")
